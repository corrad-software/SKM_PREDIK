...
const calculateMateriality = computed(() => {
  if (!materialityData.value.benchmarkValue || !materialityData.value.materialityPercentage) {
    return '0.00';
  }
...
const calculatePML = computed(() => {
  if (!calculateMateriality.value || !materialityData.value.pmlPercentage) {
    return '0.00';
  }
...
const calculateCTT = computed(() => {
  if (!calculatePML.value || !materialityData.value.cttPercentage) {
    return '0.00';
  }
...
const calculateMaterialityResults = () => {
  // Validate inputs before showing results
  if (!materialityData.value.benchmark || 
      !materialityData.value.benchmarkValue || 
      !materialityData.value.materialityPercentage || 
      !materialityData.value.pmlPercentage || 
      !materialityData.value.cttPercentage) {
      ...
      <div v-if="selectedGroup">
        ...
        >
          ...
          <div class="p-4 overflow-y-auto h-full pb-20">
            ...
            <!-- Calculate Button -->
            <button 
              @click="calculateMaterialityResults"
